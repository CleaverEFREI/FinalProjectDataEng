...
def test_homepage(homepage):
    assert (
        requests.get(homepage).text.split("<h1>")[1].split("</h1>")[0]
...
def test_model(homepage):
    ...
    response = requests.post(homepage + "/predict", data=data[0])
    ...
    response = requests.post(homepage + "/predict", data=data[1])
    rep_html = response.text.split("h2")[1].split("{")[1]
    ...
    response = requests.post(homepage + "/predict", data=data[2])
    rep_html = response.text.split("h2")[1].split("{")[1]
    insult = float(rep_html.split("insult&#39;: ")[1].split(",")[0])
    threat = float(rep_html.split("threat&#39;: ")[1].split(",")[0])
    obscene = float(rep_html.split("obscene&#39;: ")[1].split(",")[0])
    ...
    response = requests.post(homepage + "/predict", data=data[3])
    rep_html = response.text.split("h2")[1].split("{")[1]
